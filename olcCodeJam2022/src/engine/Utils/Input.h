...
static bool MouseEventActive = true;
...
static bool isReleased(unsigned int keyCode)
{
	if (glfwGetKey(Application::Get()->getWindow()->getHandle(), keyCode) == GLFW_RELEASE)
		return true;
	return false;
}
...
static void Mouse(bool works)
{
	MouseEventActive = works;
}

static bool isMousePressed(unsigned int keyCode)
{
	if (glfwGetMouseButton(Application::Get()->getWindow()->getHandle(), keyCode) == GLFW_PRESS && MouseEventActive)
		return true;
	return false;
}